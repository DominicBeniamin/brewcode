            ...
            <form id="addEquipmentForm" class="space-y-6">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="vessel">Fermentation Vessel (Bucket, Carboy, Keg, etc)</option>
                        <option value="monitoring">Monitoring Device (TILT, Inkbird Controller, etc)</option>
                        <option value="lab">Lab Equipment (Hydrometer, Refractometer, pH Meter, etc)</option>
                        <option value="other">Other (Auto-siphon, corker, etc)</option>
                ...
                <div id="dynamicFields" class="space-y-6 hidden">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            max="99"
                            value="1"
                            class="w-24 bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                        />
                    </div>
                    ...
                    <div id="capacityContainer" class="hidden">
                        ...
                        <div class="flex gap-2">
                            <input 
                                ...
                                min="0.1"
                                placeholder="0.0"
                                class="w-32 bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                            />
                            <select 
                                name="capacityUnit"
                                id="capacityUnit"
                                class="w-40 bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                            >
                                <option value="l">Litres (L)</option>
                                <option value="gal">US Gallons (gal)</option>
                                <option value="imp-gal">Imperial Gallons (gal)</option>
                            </select>
                        </div>
                    </div>
            ...
            <form id="editEquipmentForm" class="space-y-6">
                ...
                <div>
                    ...
                    <select 
                        ...
                        class="w-full bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                    >
                        <option value="vessel" ${equipmentType === 'vessel' ? 'selected' : ''}>Fermentation Vessel</option>
                        <option value="monitoring" ${equipmentType === 'monitoring' ? 'selected' : ''}>Monitoring Device</option>
                        <option value="lab" ${equipmentType === 'lab' ? 'selected' : ''}>Lab Equipment</option>
                        <option value="other" ${equipmentType === 'other' ? 'selected' : ''}>Other</option>
                        <option value="custom" ${equipmentType === 'custom' ? 'selected' : ''}>Custom</option>
                    </select>
                </div>
                ...
                <div id="dynamicFields" class="space-y-6">
                    ...
                    <div id="capacityContainer" class="${equipmentType === 'vessel' ? '' : 'hidden'}">
                        ...
                        <div class="flex gap-2">
                            ...
                            <select 
                                ...
                                class="w-40 bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                            >
                                <option value="l" ${displayCapacityUnit === 'l' ? 'selected' : ''}>Litres (L)</option>
                                <option value="gal" ${displayCapacityUnit === 'gal' ? 'selected' : ''}>US Gallons (gal)</option>
                                <option value="imp-gal" ${displayCapacityUnit === 'imp-gal' ? 'selected' : ''}>Imperial Gallons (gal)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="materialContainer" class="${equipmentType === 'vessel' ? '' : 'hidden'}">
                        ...
                        <select 
                            ...
                            class="w-full bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                        >
                            <option value="" ${!equipment.material ? 'selected' : ''}>Not specified</option>
                            <option value="Plastic" ${equipment.material === 'Plastic' ? 'selected' : ''}>Plastic</option>
                            <option value="Glass" ${equipment.material === 'Glass' ? 'selected' : ''}>Glass</option>
                            <option value="Stainless Steel" ${equipment.material === 'Stainless Steel' ? 'selected' : ''}>Stainless Steel</option>
                        </select>
                    </div>
                    ...
                    <div id="calibrationContainer" class="${equipmentType === 'lab' ? '' : 'hidden'}">
                        ...
                        <div class="flex gap-2">
                            ...
                            <select 
                                ...
                                class="w-24 bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:outline-none"
                            >
                                <option value="c" ${equipment.calibrationTempUnit === 'c' ? 'selected' : ''}>°C</option>
                                <option value="f" ${equipment.calibrationTempUnit === 'f' ? 'selected' : ''}>°F</option>
                            </select>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">
                            Leave blank if not applicable or unknown
